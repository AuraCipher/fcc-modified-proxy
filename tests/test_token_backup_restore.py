...
def test_indefinite_retention_and_backup():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("core.token_tracking._get_db_path", mock_get_db_path):
            ...
            tracker1.add_tokens(
                "nvidia_nim", "z-ai/glm4.7", input_tokens=1000, output_tokens=1500
            )
            tracker1.add_tokens(
                "openrouter", "gpt-4-turbo", input_tokens=800, output_tokens=1200
            )
            tracker1.add_tokens(
                "deepseek", "deepseek-v3", input_tokens=600, output_tokens=900
            )

            # More data (simulating ongoing usage)
            tracker1.add_tokens(
                "nvidia_nim", "z-ai/glm4.7", input_tokens=500, output_tokens=750
            )
            tracker1.add_tokens(
                "openrouter", "gpt-4", input_tokens=300, output_tokens=450
            )

            report1 = tracker1.get_report()
            total1 = report1["total"]

            print("✓ Device 1 tracked:")
            ...
            print("  Retention: ✅ INDEFINITE (no auto-delete)")
            ...
            print(
                f"  Models tracked: {sum(len(p['by_model']) for p in backup_data['by_provider'].values())} total"
            )
            ...
            with patch("core.token_tracking._get_db_path", mock_get_db_path2):
                ...
                assert total2["input_tokens"] == total1["input_tokens"], (
                    "Input tokens mismatch!"
                )
                assert total2["output_tokens"] == total1["output_tokens"], (
                    "Output tokens mismatch!"
                )
                assert total2["request_count"] == total1["request_count"], (
                    "Request count mismatch!"
                )

                # Check providers
                assert len(report2_restored["by_provider"]) == len(
                    report1["by_provider"]
                )
                ...
                print(
                    f"  All {len(report2_restored['by_provider'])} providers restored"
                )
                print("  All models in each provider restored")

                # ===== PHASE 6: Continue Usage on Device 2 =====
                print("\n" + "=" * 70)
                print(
                    "PHASE 6: Add more data on Device 2 (continuing from backup point)"
                )
                print("=" * 70)

                tracker2.add_tokens(
                    "nvidia_nim", "z-ai/glm4.7", input_tokens=400, output_tokens=600
                )
                report2_updated = tracker2.get_report()
                total2_updated = report2_updated["total"]

                print("✓ New data added on Device 2:")
                print(f"  Previous total: {total2['total_tokens']} tokens")
                print("  New batch: 400 + 600 = 1,000 tokens")
                print(f"  New total: {total2_updated['total_tokens']} tokens")
                print(
                    f"  Requests: {total2['request_count']} → {total2_updated['request_count']}"
                )
