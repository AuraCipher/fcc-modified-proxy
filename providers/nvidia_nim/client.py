...
class NvidiaNimProvider(OpenAIChatTransport):
    ...
    def __init__(
        ...
        *,
        ...
        if self._use_pooled_keys:
            ...
            logger.info("NIM using single API key")
    ...
    async def _create_stream(self, body: dict) -> tuple[Any, dict]:
        """Create a streaming chat completion, optionally retrying once per key."""
    ...
    async def _create_stream_direct(self, body: dict) -> tuple[Any, dict]:
        """Original single-key path (no pool acquire / switch delays)."""
        try:
            create_body = self._prepare_create_body(body)
            stream = await self._global_rate_limiter.execute_with_retry(
