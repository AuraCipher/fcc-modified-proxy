...
class AnthropicMessagesTransport(BaseProvider):
    ...
    async def _send_stream_request(self, body: dict) -> httpx.Response:
        """Create a streaming messages response with smart proxy selection.
        ...
        to direct computer IP immediately — **no sequential retry loop**.
        """
        ...
        if proxy_url is not None:
            return await self._send_via_proxy(body, proxy_url, pool)

        # No healthy proxy available → direct IP
        return await self._send_direct(body)

    async def _send_via_proxy(
        self, body: dict, proxy_url: str, pool: ProxyPool
    ) -> httpx.Response:
        """Try to send the request via *proxy_url*; fall back to direct IP on failure."""
        import time
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                # Handle rate limit (429) — set cooldown
                ...
                # Handle proxy auth failure (407)
                if response.status_code == 407:
                    logger.warning(
                        "PROXY_POOL: {} proxy auth failure (407) via {}, falling back to direct IP",
                        self._provider_name,
                        proxy_label,
                    )
                    pool.report_failure(proxy_url, "http_407")
                    await response.aclose()
                    return await self._send_direct(body)

                # Detect fake 200 OK containing error payloads
                if response.status_code == 200:
                    ...
                    if preview_bytes:
                        ...
                        if any(sig in preview_text for sig in error_signatures):
                            logger.warning(
                                "PROXY_POOL: {} fake 200 OK with error payload via {}, falling back to direct IP",
                                self._provider_name,
                                proxy_label,
                            )
                            pool.report_failure(proxy_url, "fake_200_error")
                            await response.aclose()
                            return await self._send_direct(body)

                # Success — keep proxy transport alive on response so it
                # doesn't get GC'd while the caller streams the body.
                # Then replace client._transport with a dummy so the
                # ``async with`` exit doesn't close the real transport.
                response._proxy_transport = client._transport
            ...
            except (httpx.ProxyError, httpx.ConnectTimeout, httpx.ReadTimeout) as exc:
                logger.warning(
                ...
                logger.warning(
                    "PROXY_POOL: {} unexpected {} via {}, falling back to direct IP",
                    self._provider_name,
                    type(exc).__name__,
                    proxy_label,
                )
    ...
    async def _send_direct(self, body: dict) -> httpx.Response:
        """Send request directly (no proxy) using the default client."""
        set_request_proxy("direct")
        logger.debug(
            "PROXY_POOL: {} using direct computer IP",
            self._provider_name,
        )
